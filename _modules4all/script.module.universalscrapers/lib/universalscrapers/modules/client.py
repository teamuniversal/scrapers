...
'''
    UNIVERSAL SCRAPERS
    credits to Exodus-Covenant developers
...
from universalscrapers.modules import cache
from universalscrapers.modules import workers
from universalscrapers.modules import dom_parser
...
def request(url, close=True, redirect=True, error=False, verify=True, proxy=None, post=None, headers=None, mobile=False,
    ...
    try:
        ...
        if verify == True and ((2, 7, 8) < sys.version_info < (2, 7, 12) or platform.uname()[1] == 'XboxOne'):
            try:
                import ssl
        ...
        try:
            ...
            if response.code == 503:
                ...
                if 'cf-browser-verification' in cf_result:
                    ...
                    xbmc.log('Request-Error (%s): %s' % (str(response.code), url), xbmc.LOGDEBUG)
...
class cfcookie:
    ...
    def _get_cookie(self):
        try:
            ...
            cookie = cfscrape.get_cookie_string(self.netloc, user_agent=self.ua)
            if 'cf_clearance' in cookie[0]: self.cookie = cookie[0]
        except:
            pass
