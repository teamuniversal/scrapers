...
class skyt(Scraper):
    domains = ['https://www.skytorrents.lol']
    name = "Sky Torrents"
    sources = []
    ...
    def scrape_movie(self, title, year, imdb, debrid = False):
        try:
            ...
            start_url = '%s/?query=%s %s&category=movie' % (self.base_link, urllib.quote_plus(search_id),year)
    ...
    def scrape_episode(self, title, show_year, year, season, episode, imdb, tvdb, debrid=False):
        try:
            ...
            start_url='%s/?query=%s&category=show' %(self.base_link, query)
            #print start_url
    ...
    def get_source(self,start_url,title,year,season,episode,start_time):
        try:
            ...
            #print r
            Endlinks=re.compile('torrent" rel="nofollow".+?img alt="(.+?)".+?href="(.+?)".+?class="is-hidden-touch">(.+?)</td>',re.DOTALL).findall(r)
            #print 'scraperchk - scrape_movie - EndLinks: '+str(Endlinks)
            for qual, Magnet, size in Endlinks:
                Magnet=Magnet.replace('%3A',':').replace('%3F','?').replace('%3D','=').split('&dn=')[0]
                print Magnet + '<><><><><>'
